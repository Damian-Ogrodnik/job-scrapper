...
module.exports.getAllOffers = async (city, category, numberOfPages) => {
    ...
    const olxData = [OLXlink, 'OLX', city, numberOfPages, 'table#offers_table > tbody > .wrap', '.offer-wrapper', 'h3', 'h3 > a', null, 'div > p > small > span', 'tr > td > div > p', '.next > a']
    const pracujData = [pracujLink, 'pracuj.pl', city, numberOfPages, '#results > ul > .results__list-container-item', 'div  > .offer__info', 'h3', '.offer-details__title-link', 'p', '.offer-labels__item', '.offer-labels:last-of-type', 'li.pagination_element--next > a']
